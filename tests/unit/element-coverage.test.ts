...
describe('facet checklists (R1.1)', () => {
  it('gives every facet at least three elements', () => {
    for (const f of FACETS) {
      expect(f.elements.length, `facet ${f.id} ${f.name}`).toBeGreaterThanOrEqual(3);
    }
  });

  it('keeps element ids globally unique and facet-bound', () => {
  ...
  it('labels elements in plain language — no facet jargon leaks to the rail', () => {
...
describe('meter derived from the checklist (R1.1)', () => {
  it('is pending while every element is outstanding', () => {
    expect(deriveFacetState(els('outstanding', 'outstanding'))).toBe('pending');
  });

  it('is partial once some but not all elements are closed', () => {
  ...
  it('is answered only when every element is closed', () => {
  ...
  it('is not_applicable only when the whole checklist is ruled out', () => {
  ...
  it('lets an honest unknown close a facet with elements still outstanding (P3)', () => {
  ...
  it('reports counts, not a bare percentage', () => {
  ...
  it('treats captured and not_applicable as closed, outstanding as open', () => {
  ...
  it('cannot report answered while anything is outstanding — the meter cannot overclaim', () => {
    for (const f of FACETS) {
