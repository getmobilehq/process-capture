...
describe('information-value ordering (R9.2 / R4.2 — one implementation)', () => {
  it('puts a conflicting claim above everything else', () => {
  ...
  it('prefers the mandatory-core facets when nothing conflicts', () => {
  ...
  it('prefers finishing a nearly-complete facet over starting a fresh one', () => {
  ...
  it('never re-asks a facet the informant has honestly closed (P3)', () => {
  ...
  it('gives every candidate a reason it can cite (R4.2)', () => {
    const ranked = rankCandidates({ coverage: coverageAll('pending'), elements: allOutstanding });
    expect(ranked.every((c) => c.because.trim().length > 0)).toBe(true);
  });

  it('asks at most two follow-ups a turn — a listener, not a questionnaire (R4.2)', () => {
    expect(selectFollowUps({ coverage: coverageAll('pending'), elements: allOutstanding })).toHaveLength(2);
  });

  it('returns nothing to ask once every element is closed', () => {
...
describe('open items from a truncated interview (R9.3)', () => {
  it('names the facet and the element for every gap, as a follow-up seed list', () => {
  ...
  it('lists nothing when the checklist is closed', () => {
    const closed = ALL_ELEMENTS.map((e) => ({
      facetId: e.facetId,
      elementId: e.id,
      state: 'captured' as ElementStateValue,
