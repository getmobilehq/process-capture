...
describe('data-loss protection (R10.3)', () => {
  it('persists a draft as it is typed, so a crash loses seconds not the answer', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);

    await saveDraft({ sessionId: session.id, seq: 2, content: 'We log the comp' }, db);
    await saveDraft({ sessionId: session.id, seq: 2, content: 'We log the complaint in the CRM' }, db);

    const active = await getActiveDraft(session.id, db);
  ...
  it('restores the exact prior state on reopening — the recovery banner case', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);
    const content = 'Four minutes of transcription that must survive a tab close.';
    await saveDraft({ sessionId: session.id, seq: 2, content, origin: 'voice' }, db);

    // Simulate reopening the interview URL: read back what was unsubmitted.
    const recovered = await getActiveDraft(session.id, db);
    expect(recovered?.content).toBe(content);
    expect(recovered?.origin).toBe('voice');
  });

  it('makes discard reversible — Undo restores the draft byte-identically', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);
    const content = 'Discard 4 minutes of transcription? — this text must come back exactly.';
    await saveDraft({ sessionId: session.id, seq: 2, content }, db);

    await discardDraft(session.id, db);
    expect(await getActiveDraft(session.id, db)).toBeUndefined();
  ...
  it('never hard-deletes: a discarded draft is still on disk, awaiting Undo', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);
    await saveDraft({ sessionId: session.id, seq: 2, content: 'still here' }, db);
    await discardDraft(session.id, db);

    const discarded = await getUndoableDraft(session.id, db);
    expect(discarded?.content).toBe('still here');
    expect(discarded?.status).toBe('discarded');
  });

  it('archives the prior take on re-record rather than overwriting it', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);
    const firstTake = 'The first take, which must survive being replaced.';
    await saveDraft({ sessionId: session.id, seq: 2, content: firstTake, origin: 'voice' }, db);

    const fresh = await startNewTake(session.id, 2, db);
    expect(fresh.content).toBe('');
    expect(fresh.take).toBe(2);

    const archived = await getArchivedTakes(session.id, 2, db);
    expect(archived).toHaveLength(1);
    expect(archived[0].content).toBe(firstTake);
  });

  it('cannot destroy a transcription in two actions — discard then undo round-trips', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);
  ...
  it('keeps exactly one active draft after an undo, so recovery is unambiguous', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);
    await saveDraft({ sessionId: session.id, seq: 2, content: 'first' }, db);
    await discardDraft(session.id, db);
  ...
  it('retires the draft once its turn is submitted, so it is not offered for recovery', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);
    await saveDraft({ sessionId: session.id, seq: 2, content: 'sent' }, db);
    await markDraftSubmitted(session.id, 2, db);
    expect(await getActiveDraft(session.id, db)).toBeUndefined();
  });

  it('starts a clean draft for the next question without touching the last one', async () => {
    const { db } = await makeTestDb();
    const { session } = await makeSessionFixture(db);
    await saveDraft({ sessionId: session.id, seq: 2, content: 'answer to Q1' }, db);
    await saveDraft({ sessionId: session.id, seq: 4, content: 'answer to Q2' }, db);

    const active = await getActiveDraft(session.id, db);
    expect(active?.seq).toBe(4);
    expect(active?.content).toBe('answer to Q2');
    // Nothing was destroyed on the way.
    expect(await getUndoableDraft(session.id, db)).toBeUndefined();
  });
});
