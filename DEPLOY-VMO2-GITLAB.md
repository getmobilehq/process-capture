...
| **May we enable Anthropic models in Vertex AI Model Garden?** | It is how the interview model runs without a third-party API key | Stay on the direct Anthropic API, which needs a commercial agreement and egress to `api.anthropic.com` |
| **Is `europe-west1` acceptable for interview content?** | `europe-west2` serves no Anthropic model, so Claude on Vertex means EU rather than UK | Either accept EU, or stay on the direct API — which is neither UK nor inside GCP, so it is not the safer option it sounds like |

Also settle two things internally:

- **Whose Anthropic account pays?** On the direct API the key must be VMO2's, not
  yours — a pilot running on a personal key is a conversation you do not want to
  have later. On Vertex the question disappears: there is no key, and the spend is
  on the GCP invoice. See §11.1.
...
### 11.1 Choosing where the interview model runs

Two routes to the same Claude models. The default is unchanged, so doing nothing
keeps today's behaviour.

| | `model_provider = "anthropic"` (default) | `model_provider = "vertex"` |
|---|---|---|
| Credential | API key in Secret Manager | None — the service account |
| Network | Cloud Run egresses to `api.anthropic.com` | Stays inside Google's network |
| Procurement | Separate agreement with Anthropic | On the existing GCP invoice |
| Region | Anthropic's infrastructure | `europe-west1` — EU, **not** UK |
| Model id | `claude-sonnet-4-6` | `claude-sonnet-4-5@20250929` — Vertex names the version |

To switch:

1. **Enable the models in Model Garden.** Vertex AI → Model Garden → "Claude
   Sonnet 4.5" → Enable. It is a terms acceptance, so it needs someone who may
   accept terms for VMO2. **Until this is done every call returns 404**, which is
   confusing precisely because it looks like a wrong model id rather than a
   missing entitlement.
2. Set in `envs/vmo2.tfvars`:
   ```hcl
   model_provider      = "vertex"
   vertex_model_region = "europe-west1"
   model               = "claude-sonnet-4-5@20250929"
   ```
3. Plan and deploy. Terraform stops creating the `magpie-anthropic-api-key`
   secret, so there is one fewer value to set in §11.
4. Run one interview turn and check it answers. Vertex applies its own per-model
   quota, separate from anything you have on the direct API, and it starts low —
   check it before a pilot with several concurrent interviews.

Rolling back is the same three lines in reverse, plus putting the key back.
...
| `ANTHROPIC_API_KEY` | from Secret Manager — **you set the value** (§11). Not created at all on the Vertex path |
| `MODEL_PROVIDER` | `anthropic` (direct API) or `vertex` (Model Garden) — see §11.1 |
| `VERTEX_MODEL_REGION` | region serving the interview model; separate from the transcription region because europe-west2 serves no Anthropic model |
